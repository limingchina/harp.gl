...
function procesStringEnocodedNumeralInterpolatedProperty(
    ...
    let needsMask = false;
    if (
        format.type === StringEncodedNumeralType.Meters ||
        format.type === StringEncodedNumeralType.Pixels
    ) {
        for (let elementIdx = 0; elementIdx < prop.values.length; ++elementIdx) {
            for (
                let formatIdx = StringEncodedNumeralType.Meters;
                formatIdx <= StringEncodedNumeralType.Pixels;
                ++formatIdx
            ) {
                if (
                    StringEncodedNumeralFormats[formatIdx].regExp.test((prop.values[
                        elementIdx
                    ] as unknown) as string)
                ) {
                    maskValues[elementIdx] =
                        formatIdx === StringEncodedNumeralType.Pixels ? 1.0 : 0.0;
                    needsMask = needsMask || maskValues[elementIdx] === 1.0;

                    const result = StringEncodedNumeralFormats[formatIdx].decoder((prop.values[
                        elementIdx
                    ] as unknown) as string);
                    for (let i = 0; i < result.length; ++i) {
                        propValues[elementIdx * StringEncodedNumeralFormats[formatIdx].size + i] =
                            result[i];
                    }
                    break;
                }
        ...
        format.type === StringEncodedNumeralType.Hex ||
        format.type === StringEncodedNumeralType.RGB ||
        format.type === StringEncodedNumeralType.HSL
    ) {
        for (let elementIdx = 0; elementIdx < prop.values.length; ++elementIdx) {
            for (
                let formatIdx = StringEncodedNumeralType.Hex;
                formatIdx <= StringEncodedNumeralType.HSL;
                ++formatIdx
            ) {
                if (
                    StringEncodedNumeralFormats[formatIdx].regExp.test((prop.values[
